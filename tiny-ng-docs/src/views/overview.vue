<template>
  <div>
    <div ref="overviewContainer" class="overview-layout pt48">
      <h1 class="mb20 f24 fw-600">组件总览</h1>
      <h1 class="f14 my20 lh21">TinyNG 为 Web 应用提供了丰富的基础 UI 组件，我们还将持续探索企业级应用的最佳 UI 实践，欢迎尝试使用 TinyNG。</h1>
      <!-- 搜索 -->
          ...
          <div v-for="(cell, cellIndex) in menu.children" :label="cell" :key="cellIndex" class="my10 text-center overview-card-container">
            <router-link :to="getTo(cell.key)" class="decoration-none">
              <n-card hoverable class="br-4">
                <img class="h125 w125" :src="$pub(`overviewimage/imglist/${cell.key}.svg`)" :onerror="`this.src='${$pub(`overviewimage/imglist/dev.svg`)}'`" />
