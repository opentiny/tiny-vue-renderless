...
const getHasComment = (state) => (comment) => {
  const childNodes = Array.from(state.parent.childNodes)
...
const getGetFragment =
  ...
  (commentFlag) => {
    ...
    commentFlag &&
      !hasComment(startComment) &&
      fragment.appendChild(document.createComment(startComment))

    state.nodes.forEach((node) => fragment.appendChild(node))

    commentFlag &&
      !hasComment(endComment) &&
      fragment.appendChild(document.createComment(endComment))
...
const getDisable =
  ...
  () => {
    ...
    Array.from(state.parent.childNodes).forEach((child, i) => {
      if (child.nodeType === 8) {
        if (
          child.textContent === startComment ||
          child.textContent === endComment
        ) {
          indices.push(i)
        }
      }
  ...
  (mutations) => {
    ...
    for (let i = 0; i < mutations.length; i++) {
      ...
      const filteredAddedNodes = Array.from(mutation.addedNodes).filter(
        (node) => !state.nodes.includes(node)
      )
...
export default ({
  ...
  defineComponent({
    ...
    setup(props) {
      ...
      const getFragment = getGetFragment({
        hasComment,
        startComment,
        state,
        endComment
      ...
      const afterUpdated = getAfterUpdated({
        state,
        instance,
        props,
        bootObserver,
      ...
      const watchDisabled = getWatchDisabled({
        disable,
        teardownObserver,
        bootObserver,
        move
      ...
      return () =>
        h(
          'div',
          {
            ...
            style: { 'visibility:hidden;display:none;': !props.disabled }
          },
          (typeof instance?.$slots.default === 'function'
