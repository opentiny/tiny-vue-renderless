...
export const getComponentName = () => {
  ...
  let componentName =
    instance?.vnode?.componentOptions?.Ctor?.extendOptions?.name
  if (!componentName) {
    componentName =
      instance?.parent?.vnode?.componentOptions?.Ctor?.extendOptions?.name
  }
...
const filter = (name) =>
  name.indexOf('$') === 0 || name.indexOf('_') === 0 || name === 'constructor'
...
export const createComponentFn = (design) => {
  return ({ component, propsData, el }) => {
    const comp = Object.assign(component, {
      provide: { [design.configKey]: design.configInstance }
    })
