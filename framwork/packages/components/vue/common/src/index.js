...
export const resolveMode = (props, context) => {
  let isRightMode = (mode) =>
    ~['pc', 'mobile', 'mobile-first', 'watch'].indexOf(mode)
  let config = rootConfig(context)
  let tinyModeProp =
    typeof props.tiny_mode === 'string' ? props.tiny_mode : null
...
const resolveTheme = ({ props, context, utils }) => {
  ...
  let tinyThemeProp =
    typeof props.tiny_theme === 'string' ? props.tiny_theme : null
...
const resolveChartTheme = ({ props, context, utils }) => {
  const config = rootConfig(context)
  let tinyChartProp =
    typeof props.tiny_chart_theme === 'object' ? props.tiny_chart_theme : null
...
export const mergeClass = (...cssClasses) =>
  twMerge(stringifyCssClass(cssClasses))
...
export const setup = ({
  ...
  renderless,
  api,
  extendOptions = { framework: isVue3 ? 'Vue3' : 'Vue2' },
  mono = false,
  classes = {}
}) => {
  const render =
  ...
  const globalDesignConfig = hooks.inject(design.configKey, {})
  const designConfig =
    globalDesignConfig?.components?.[getComponentName().replace($prefix, '')]
  ...
  if (typeof designConfig?.renderless === 'function') {
    Object.assign(
      sdk,
      designConfig.renderless(props, hooks, utils, sdk, extendOptions)
    )
  }
