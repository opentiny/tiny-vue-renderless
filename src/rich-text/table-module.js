...
function addMatcherTable({ _quill, oper }) {
  _quill.clipboard.addMatcher('TABLE', (node, delta) => {
    const id = cellName()
...
function addMatcherTr({ _quill, oper }) {
  _quill.clipboard.addMatcher('TR', (node, delta) => {
    const id = cellName()
...
function addMatcherTd({ _quill, oper, Delta }) {
  _quill.clipboard.addMatcher('TD', (node, delta) => {
...
function addMatcher3(_quill) {
  _quill.clipboard.addMatcher(3, (node, delta) => {
    if (['TABLE', 'TR'].indexOf(node.parentNode.tagName) !== -1) {
      return { ops: [] }
    } else {
      return delta
    }
...
function addMatcherFont(_quill) {
  _quill.clipboard.addMatcher('FONT', (node, delta) => {
    if (node.innerText === '') {
      return { ops: [] }
    } else {
      return delta
...
function provideTableCell({ Container, Parchment, Block, BlockEmbed, ContainBlot }) {
  ...
  class TableCell extends ContainBlot {
    ...
    formatAt(index, length, name, value) {
      ...
      attrWhitelist.indexOf(name) !== -1 && this.domNode.setAttribute(name, value)
    }
