...
export const formatTime = () => (time) => (time.hours < 10 ? '0' + time.hours : time.hours) + ':' + (time.minutes < 10 ? '0' + time.minutes : time.minutes)
  ...
  if (start && end && step) {
    ...
    while (api.compareTime(current, end) <= 0) {
      result.push({
        value: current,
        disabled:
          api.compareTime(current, state.minTime || DATEPICKER.CompareOne) <= 0 || api.compareTime(current, state.maxTime || DATEPICKER.CompareHundred) >= 0
      })
