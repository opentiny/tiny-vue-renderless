...
export const handleBlur = ({ api, componentName, eventName, emit, props, state }) => (event) => {
  state.focused = false
...
export const handleFocus = ({ api, emit, state }) => (event) => {
  state.focused = true
...
export const handleInput = ({ api, emit, nextTick, state }) => (event) => {
...
export const resizeTextarea = ({ api, parent, refs, state }) => () => {
  if (isServer) {
    return
  }
...
export const setNativeInputValue = ({ api, state }) => () => {
...
export const handleCompositionEnd = ({ api, state }) => (event) => {
  if (state.isComposing) {
...
export const calcIconOffset = ({ CLASS_PREFIX, parent }) => (place) => {
...
export const handlePasswordVisible = ({ api, nextTick, state }) => () => {
  state.passwordVisible = !state.passwordVisible
  nextTick(api.focus)
}

export const getSuffixVisible = ({ parent, props, state }) => () =>
  parent.$slots.suffix || props.suffixIcon || state.showClear || props.showPassword || state.isWordLimitVisible || (state.validateState && state.needStatusIcon)
...
export const watchFormSelect = ({ emit, props, state }) => (value) => {
  if (props.isSelect) {
