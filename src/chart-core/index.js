...
export const dataHandler = ({ api, props, state, echartsLib, t, vm }) => () => {
  if (!api.chartHandler) {
    return
  }
...
export const nextTickResize = ({ api, nextTick }) => () => nextTick(api.resize)

export const resize = ({ props, vm, api }) => () => {
...
export const judgeWidthHandler = ({ props, api, vm, nextTick }) => () => {
  if (!props.judgeWidth) {
    return
  }
...
export const optionsHandler = ({ props, state, emit, echartsLib, api, vm }) => (options) => {
  if (options.tooltip) {
...
export const resizeableHandler = ({ api, state }) => (resizeable) => {
...
export const init = ({ state, props, api, refs, echartsLib, markRaw }) => () => {
  if (state.echarts) {
    return
  }
...
export const addResizeListener = ({ state, api }) => () => {
  on(window, 'resize', api.resizeHandler)
  state.once.onresize = true
}

export const removeResizeListener = ({ state, api }) => () => {
  off(window, 'resize', api.resizeHandler)
  state.once.onresize = false
}

export const addWatchToProps = ({ props, watch, api }) => () => {
 ...
 * 实现 events 的动态响应
 */
export const createEventProxy = ({ props, state }) => () => {
...
export const themeChange = ({ api, state }) => () => {
  api.clean()
  state.echarts = null
  api.init()
}

export const clean = ({ props, state, api }) => () => {
  if (props.resizeable) {
    api.removeResizeListener()
  }
...
export const mounted = ({ state, api, vm, props, watch, t }) => () => {
  state.echarts = null
...
export const changeHandler = ({ api, props }) => () => debounce(api.dataHandler, props.changeDelay)

export const resizeHandler = ({ api, props }) => () => debounce(api.resize, props.resizeDelay)
