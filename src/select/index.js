...
export const handleComposition = ({ api, nextTick, state }) => (event) => {
...
export const showTip = ({ props, state, refs }) => (show) => {
...
export const gridOnQueryChange = ({ props, refs, constants, state }) => (value) => {
...
export const defaultOnQueryChange = ({ props, state, constants, api }) => (value) => {
...
export const handleQueryChange = ({ api, constants, nextTick, props, vm, state }) => (value) => {
  const refs = vm.$refs
...
export const scrollToOption = ({ refs, constants }) => (option) => {
...
export const emitChange = ({ emit, props, state, constants }) => (value) => {
...
export const getOption = ({ props, state }) => (value) => {
  let option
  ...
  for (let i = state.cachedOptions.length - 1; i >= 0; i--) {
    ...
    const isEqual = isObject ? getObj(cachedOption.value, props.valueKey) === getObj(value, props.valueKey) : cachedOption.value === value
...
export const getPluginOption = ({ api, props, state }) => (value, isTree) => {
  const isRemote = state.filterOrSearch && props.remote && typeof props.remoteMethod === 'function'
...
export const toggleCheckAll = ({ api, emit, state }) => () => {
...
export const resetFilter = ({ state, api }) => () => {
  state.query = ''
  state.previousQuery = undefined
  api.handleQueryChange(state.query)
}

export const handleFocus = ({ emit, props, state, api }) => (event) => {
  if (!state.softFocus) {
...
export const focus = ({ refs, state }) => () => {
  if (!state.softFocus) {
    refs.reference?.focus()
  }
}

export const blur = ({ refs, state }) => () => {
  state.visible = false
  refs.reference?.blur()
}

export const handleBlur = ({ constants, dispatch, emit, state }) => (event) => {
  setTimeout(() => {
...
export const toggleLastOptionHitState = ({ state }) => (hit) => {
...
export const deletePrevTag = ({ api, constants, emit, props, state, vm }) => (event) => {
  const refs = vm.$refs
  ...
  if (event.target.value.length <= 0 && !api.toggleLastOptionHitState()) {
    ...
    if (props.renderType === constants.TYPE.Grid) {
      const rows = state.selected.slice().filter((item) => value.indexOf(item[props.valueField]) > -1)
...
export const resetInputHeight = ({ constants, nextTick, props, refs, state }) => () => {
...
export const resetHoverIndex = ({ props, state }) => () => {
  if (!props.showOverflowTooltip) {
    state.hoverIndex = -1
...
export const handleOptionSelect = ({ api, emit, nextTick, props, refs, state }) => (option, byClick) => {
...
export const setSoftFocus = ({ refs, state }) => () => {
  state.softFocus = true
...
export const getValueIndex = (props) => (arr = [], value = null) => {
  const isObject = Object.prototype.toString.call(value).toLowerCase() === '[object object]'
...
export const toggleMenu = ({ refs, state, props }) => () => {
...
export const selectOption = ({ api, state, props }) => () => {
...
export const deleteSelected = ({ api, constants, emit, props, vm, state }) => (event) => {
...
export const deleteTag = ({ api, constants, emit, props, state, vm }) => (event, tag) => {
  const refs = vm.$refs
  const isTree = props.renderType === constants.TYPE.Tree
  ...
  if (isTree && !props.treeOp.checkStrictly) {
    ...
    state.selected.slice().map((node) => treeIds.indexOf(node[props.valueField]) === -1 && treeValue.push(node[props.valueField]))
  }
...
export const onInputChange = ({ api, props, state, constants, nextTick }) => () => {
  if (!props.delay) {
...
export const resetInputWidth = ({ refs, state }) => () => {
  state.inputWidth = refs.reference?.$el?.getBoundingClientRect()?.width || 0
}

export const handleResize = ({ api, props }) => () => {
  api.resetInputWidth()
...
export const navigateOptions = ({ api, nextTick, state }) => (direction) => {
...
export const emptyText = ({ I18N, props, state, t }) => () => {
  if (props.loading) {
...
export const watchValue = ({ api, constants, dispatch, props, refs, state }) => (value, oldValue) => {
  if (props.multiple) {
    api.resetInputHeight()
...
export const calcOverFlow = ({ refs, props, state }) => (height) => {
...
export const toVisible = ({ constants, state, props, vm, api, nextTick }) => () => {
  const refs = vm.$refs
...
export const toHide = ({ constants, state, props, vm, api }) => () => {
  const refs = vm.$refs
...
export const watchVisible = ({ api, constants, emit, state, refs }) => (value) => {
  if (api.onCopying()) {
    return
  }
...
export const watchOptions = ({ api, constants, nextTick, parent, props, state }) => () => {
...
export const handleCopyClick = ({ parent, props, state }) => () => {
...
export const selectChange = ({ emit, props, state }) => ({ $table, selection, checked, row }) => {
  const { textField, valueField } = props
  ...
  if (row) {
    ...
    checked
      ? (state.selected = state.selected.concat(selection.filter((row) => !~props.modelValue.indexOf(row[valueField]))))
...
export const getcheckedData = ({ props, state }) => () => {
  const checkedKey = []
...
export const radioChange = ({ emit, props, state }) => ({ row }) => {
...
export const treeNodeClick = ({ emit, props, state }) => (data) => {
  if (!props.multiple) {
...
export const nodeCheckClick = ({ emit, props }) => (data, { checkedKeys, checkedNodes }) => {
...
export const nodeCollapse = ({ state, constants }) => () => {
  setTimeout(() => {
    state.selectEmitter.emit(constants.EVENT_NAME.updatePopper)
  }, 310)
}

export const nodeExpand = ({ state, constants }) => () => {
  setTimeout(() => {
    state.selectEmitter.emit(constants.EVENT_NAME.updatePopper)
  }, 310)
}
...
export const watchPropsOption = ({ constants, parent, props, state }) => () => {
...
export const buildSelectConfig = ({ props, state }) => () => {
...
export const buildRadioConfig = ({ props, state }) => () => {
...
export const onMouseleaveNative = ({ state }) => () => {
...
export const onCopying = ({ state, refs }) => () =>
  state.searchSingleCopy && state.selectedLabel && refs.reference && refs.reference.hasSelection && refs.reference.hasSelection()

export const watchHoverIndex = ({ state }) => (value) => {
...
export const mounted = ({ api, parent, state, props, refs }) => () => {
...
export const unMount = ({ api, parent, refs, state }) => () => {
...
export const computeOptimizeOpts = ({ props, state }) => () => {
...
export const watchOptimizeOpts = ({ api, props, refs, state }) => () => {
  const { optimizeOpts, optimizeStore } = state
...
export const computeMultipleLimit = ({ props, state }) => () => {
