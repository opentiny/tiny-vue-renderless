...
export const keyup = ({ api, props }) => ({ item, index, event }) => {
  ...
  if (api.isIP4(props.type)) {
    ...
    if ([KEY_CODE.Space, KEY_CODE.NumpadDecimal, KEY_CODE.NumpadComma].indexOf(keyCode) > -1 && value) {
...
const checkError1 = ({ Space, NumpadDecimal, NumpadComma, keyCode, value }) => [Space, NumpadDecimal, NumpadComma].indexOf(keyCode) > -1 && value
...
export const keydown = ({ api, props, state }) => ({ item, index, event }) => {
  ...
  const isfilterKeyCodes = state.filterKeyCodes.indexOf(keyCode) !== -1
  const nextIndex = index + 1
