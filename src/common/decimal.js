...
export function Decimal(num) {
  ...
  this.div = (target) => {
    ...
    const absSmallest = Math.pow(10, Math.abs(smallest))
...
export const toFixed = (num, fraction = 0) => {
  ...
  const npmPow = num.toString().length < Math.pow(2, 53).toString().length - 1 ? Math.pow(10, fraction) : Math.pow(10, fraction - 1)
