...
import { reactive, watch, markRaw, onBeforeUnmount, onDeactivated } from 'vue'
import { applyClass, getTransitionInfo } from '@opentiny/utils'
...
type FloatingEventName = 'show' | 'hide' | 'update'

/** 默认配置 */
const defaultOption: Partial<FloatOption> = {
  reference: null,
  popper: null,
  show: false,
  autoUpdate: true,
...
/** 执行一次 popper 的更新动作 */
const updatePopper = (state: FloatOption) => {
  ...
  computePosition(state.reference!, state.popper!, {
    ...
    // 应用st- 原子类： flip 会造成翻转， 但返回的placement 会记录真实的位置.
    const realPlacement = placement.split('-')[0]
    applyClass(state.popper!, 'st-top st-bottom st-left st-right', false)
    applyClass(state.popper!, 'st-' + realPlacement, true)
...
/** 执行自动更新 */
const autoUpdatePopper = (state: FloatOption) => {
...
const appendPopper = (state: FloatOption) => {
  ...
  if (state.popper) {
    ...
    if (state.animate) {
      ...
      setTimeout(() => state.popper!.classList.remove(enterName), 0)
...
const closePopper = (state: FloatOption, cleanup?: () => void) => {
  if (cleanup) {
    cleanup()
  }
  // 如果已经关闭了，则忽略
  ...
  if (state.popper) {
    ...
    if (state.animate && state.animateName) {
      ...
      state.popper.classList.add(activeName)
      setTimeout(() => state.popper!.classList.add(leaveName), 0)
...
/** 触发事件 */
const emit = (state: FloatOption, eventName: FloatingEventName, params?: any) => {
...
export const useFloating = (option: Partial<FloatOption> = {}) => {
  const state = reactive(option) as FloatOption
  ...
  const watchState = () => {
    ...
    state._last.show = (state.popper && state.reference && state.show) as boolean // 真实的是否show变量
  ...
  /** 添加事件监听： show, hide, update  */
  const on = (eventName: FloatingEventName, cb: Function) => state._events[eventName].push(cb)
  /** 移除事件监听 */
  const off = (eventName: FloatingEventName, cb?: Function) =>
    cb ? (state._events[eventName] = state._events[eventName].filter((i) => i !== cb)) : (state._events[eventName] = [])
