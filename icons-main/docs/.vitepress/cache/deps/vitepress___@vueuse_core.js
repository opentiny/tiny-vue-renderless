...
function syncRefs(source, targets, options = {}) {
  ...
  return watch(source, (newValue) => targetsArray.forEach((target) => (target.value = newValue)), { flush, deep, immediate })
}
...
function onElementRemoval(target, callback, options = {}) {
  const { window: window2 = defaultWindow, document: document2 = window2 == null ? void 0 : window2.document, flush = 'sync' } = options
...
function useBrowserLocation(options = {}) {
  ...
  const buildState = (trigger) => {
    ...
    for (const key of WRITABLE_PROPERTIES) refs[key].value = (_a = window2 == null ? void 0 : window2.location) == null ? void 0 : _a[key]
    return reactive({
      trigger,
      state: state2,
      length,
...
function useClipboard(options = {}) {
  ...
  function legacyRead() {
    var _a, _b, _c
    return (_c = (_b = (_a = document == null ? void 0 : document.getSelection) == null ? void 0 : _a.call(document)) == null ? void 0 : _b.toString()) != null
      ? _c
      : ''
  }
...
function useColorMode(options = {}) {
  ...
  const store =
    storageRef || (storageKey == null ? toRef2(initialValue) : useStorage(storageKey, initialValue, storage, { window: window2, listenToStorageChanges }))
...
function useRefHistory(source, options = {}) {
  ...
  const { ignoreUpdates, ignorePrevAsyncUpdates, stop } = watchIgnorable(source, commit, { deep, flush, eventFilter: composedFilter })
...
function useDeviceMotion(options = {}) {
  ...
  const requirePermissions = useSupported(
    () => isSupported.value && 'requestPermission' in DeviceMotionEvent && typeof DeviceMotionEvent.requestPermission === 'function'
  )
...
function useDevicesList(options = {}) {
  const { navigator: navigator2 = defaultNavigator, requestPermissions = false, constraints = { audio: true, video: true }, onUpdated: onUpdated2 } = options
  const devices = ref([])
  ...
  useEventListener(
    ...
    () => {
      ...
      return (_focused.value =
        (_c = (_b = (_a = targetElement.value) == null ? void 0 : _a.matches) == null ? void 0 : _b.call(_a, PSEUDO_CLASS_FOCUS_WITHIN)) != null ? _c : false)
    },
    listenerOptions
  )
  return { focused }
...
function useFullscreen(target, options = {}) {
  ...
  const exitMethod = computed(() => {
    return ['exitFullscreen', 'webkitExitFullscreen', 'webkitExitFullScreen', 'webkitCancelFullScreen', 'mozCancelFullScreen', 'msExitFullscreen'].find(
  ...
  const fullscreenElementMethod = ['fullscreenElement', 'webkitFullscreenElement', 'mozFullScreenElement', 'msFullscreenElement'].find(
    (m) => document2 && m in document2
  )
  const isSupported = useSupported(
...
function useParallax(target, options = {}) {
  ...
  const { elementX: x, elementY: y, elementWidth: width, elementHeight: height } = useMouseInElement(target, { handleOutside: false, window: window2 })
...
function usePointerSwipe(target, options = {}) {
  ...
  const eventIsAllowed = (e) => {
    ...
    return (_c = (_b = (_a = options.pointerTypes) == null ? void 0 : _a.includes(e.pointerType)) != null ? _b : isReleasingButton || isPrimaryButton) != null
      ? _c
      : true
  }
  const listenerOptions = { passive: true }
...
function useVModel(props, key, emit, options = {}) {
  ...
  const _emit =
    ...
    ((_c = (_b = vm == null ? void 0 : vm.proxy) == null ? void 0 : _b.$emit) == null ? void 0 : _c.bind(vm == null ? void 0 : vm.proxy))
  let event = eventName
  if (!key) {
    key = 'modelValue'
  }
