<template>
  <div class="application-list">
    <tiny-grid ref="grid" :fetch-data="state.fetchData" :pager="state.pagerConfig">
      ...
      <tiny-grid-column field="tenant_id" title="组织名称"></tiny-grid-column>
      <tiny-grid-column field="project_desc" title="组织简介"></tiny-grid-column>
  ...
  setup(props) {
    ...
    const getApplicationList = ({ page, filterArgs }) => {
      ...
      return new Promise((resolve) => {
        fetchApplicationList({ action }).then((data) => {
    ...
    const pass = (row) => {
      requestUpdataApply(row.id, { action, status: APPLY_STATUS.Pass })
        .then((data) => {
    ...
    const reject = (row) => {
      requestUpdataApply(row.id, { action, status: APPLY_STATUS.Reject })
        .then((data) => {
