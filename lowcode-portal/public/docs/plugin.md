...
在设计器中，插件的范围包括顶部工具栏、左侧插件栏。插件也是一个元应用
![插件示意图](md-only-prefix/img/docimg/plugin1.png)
...
- 对于左侧插件，我们提供了 `PluginPanel` 和 `PluginSetting` 两个组件给用户使用
  ...
  ![pluginPanel示意图](md-only-prefix/img/docimg/plugin2.png)

  `PluginPanel` 用于点击左侧插件后展开的面板，分为 header 区域和 content 区域，其中 Header 区域主要显示插件标题、关闭按钮，具体使用方法请看下一节

  - PluginSetting

  ![pluginSetting 示意图](md-only-prefix/img/docimg/plugin3.png)
...
头部插槽示意图(天蓝色背景区域)：
![头部插槽示意图](md-only-prefix/img/docimg/plugin4.png)

`isCloseLeft: false` 默认关闭按钮在 Header插槽后面

![closeLeft示意图](md-only-prefix/img/docimg/plugin5.png)

`isCloseLeft: true` 关闭按钮在 title 后面

![closeLeft示意图](md-only-prefix/img/docimg/plugin6.png)
...
头部插槽示意图（红框区域）：
![头部插槽示意图](md-only-prefix/img/docimg/plugin7.png)
...
详细的协议规范、数据含义及类型请阅读[低代码协议](portal-only-prefix/platform-center/#/protocol)
