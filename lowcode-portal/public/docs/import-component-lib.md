...
## 前置介绍
可拖拽到画布中渲染的我们称之为物料，物料来源于内置组件包和物料资产包，物料资产包包含组件和区块。物料资产包的配置参考[物料资产包协议](portal-only-prefix/platform-center/#/protocol)。
...
### 配置组件
在前置介绍，我们知道了在画布中渲染组件需要将组件配置到物料资产包中，根据[物料资产包协议](portal-only-prefix/platform-center/#/protocol)，配置包含两大类：组件配置components、组件面板配置snippets。下面将在协议基础上对**核心字段**作进一步的解释，可以打开源码中物料资产包文件（packages\design-core\public\mock\bundle.json）逐个理解。
...
![property]( md-only-prefix/img/docimg/property.png)
...
![events]( md-only-prefix/img/docimg/events.png)
...
![slots]( md-only-prefix/img/docimg/slots.png)
...
![snippets]( md-only-prefix/img/docimg/snippets.png)
