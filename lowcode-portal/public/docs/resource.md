...
同样，在低代码开发中，我们不可避免的需要编写一些高代码进行组合开发，这里同样存在存储一些公共函数以供各处调用需求。工具类方法 utils
就是诞生在这样的背景之下，我们可以将一些可复用的公共函数编写到工具类方法中，也可以将一些 npm 包引用到utils中，供后续调用。
...
![添加工具类教程图](md-only-prefix/img/docimg/use-jsPlugin.png)
...
对于一些简单的公共函数来说，直接添加 function 工具类会很方便，但是对于一些比较复杂的公共函数或者第三方的一些公共函数来说，直接编写函数并不是一个理想的方式，所以，我们还提供了
npm 工具类，以供引入 npm 包。

![添加npm工具类](md-only-prefix/img/docimg/utils2.png)
...
![添加 lodash 工具类示例](md-only-prefix/img/docimg/utils3.png)
...
![效果](md-only-prefix/img/docimg/utils4.png)

### 添加 function utils

1. 点击添加工具类，选择 function ，填入名称和函数

![添加Function示意图](md-only-prefix/img/docimg/utils5.png)
