...
![npm官网 access token入口](md-only-prefix/img/docimg/npm1.png)

![创建Classic Token](md-only-prefix/img/docimg/npm2.png)

![npm包发布](md-only-prefix/img/docimg/npm3.png)

生成token 后， 配置 `NPM_AUTH_TOKEN` 环境变量, 具体配置方法参看 [此文档](portal-only-prefix/help-center/course/dev/51)
...
![修改unpkg链接位置](md-only-prefix/img/docimg/backend_code3.png) 
...
![修改区块包名](md-only-prefix/img/docimg/backend_code.png) 
...
npm 私仓部署更具公司或团体内部的npm仓库呈现形式，获取具有publish 权限 access token, 然后配置 `NPM_AUTH_TOKEN` 环境变量，配置方法可查看 [此文档](portal-only-prefix/help-center/course/dev/51)
...
执行流程如下：
  ...
  ![创建区块](md-only-prefix/img/docimg/create_block.png)

  - 编辑区块并保存

  ![编辑区块并保存](md-only-prefix/img/docimg/save_block.png)

  - 区块发布

  ![区块发布](md-only-prefix/img/docimg/publish_block.png)
    
    输入区块发布版本及描述
   
  ![输入区块发布版本及描述](md-only-prefix/img/docimg/publish_block2.png)
    
    发布完之后，就能将区块添加到物料中

  - 添加区块到物料
  
  ![添加区块](md-only-prefix/img/docimg/add_block.png)
    
    点击可选区块列表刚才发布的区块，移动到右侧已选区块列表，点击保存

  ![保存添加区块](md-only-prefix/img/docimg/add_block1.png)
    
    至此就可以在物料的区块里找到自己发布的区块并使用了

  ![使用区块](md-only-prefix/img/docimg/add_block2.png)
