...
在TinyEngine开源后，对私有化部署存在诉求的用户越来越多，而当前TinyEngine多项内容都依托在公网中，当前官网提供的[区块发布方案](portal-only-prefix/help-center/course/dev/52)，为公网环境下的发布，不能完全满足部分用户的需求。因此需要提供用户能够在内网环境下，也能够正常使用TinyEngine全部功能的方案。
...
![私仓架构图](md-only-prefix/img/docimg/block-publish-12.png)
...
![测试私仓](md-only-prefix/img/docimg/block-publish-9.png)

![测试私仓](md-only-prefix/img/docimg/block-publish-10.png)

发布完成后， 访问 [http://localhost:4873/](http://localhost:4873/) ，在 verdaccio 的管理界面能看到发布上去的npm包表示安装成功。 

![测试私仓](md-only-prefix/img/docimg/block-publish-11.png)
...
unpkg的搭建，大家可以自行在网上参考相关文档，将官方的unpkg源码下载下来，进行改造，也可以参考别人的成功案例。改造成功后，访问 [http://localhost:8080/](http://localhost:8080/) 能看到 unpkg 服务启动成功![image-20240416214349551](md-only-prefix/img/docimg/block-publish-2.png)

  

然后访问之前发布的测试包： `http://localhost:8080/@公司码/组件名` ，如能显示 js 文件内容, 则表示 unpkg 安装成功。

![image-20240417002104364](md-only-prefix/img/docimg/block-publish-3.png)
...
```
![代码片段](md-only-prefix/img/docimg/block-publish-8.png)
...
![image-20240417002104364](md-only-prefix/img/docimg/block-publish-4.png)

然后发布到私仓:

![image-20240417002104364](md-only-prefix/img/docimg/block-publish-5.png)

发布完成之后，可以打开 [http://localhost:4873/](http://localhost:4873/) 验证是否发布成功

![image-20240417002104364](md-only-prefix/img/docimg/block-publish-6.png)

并且查看本地启动的unpkg，是否能够正常访问刚才发布的区块。

![image-20240417002104364](md-only-prefix/img/docimg/block-publish-7.png)
...
1. 后端用postman调用访问文件链接效果如下：
  ...
  ![block-publish-db-1](md-only-prefix/img/docimg/block-publish-db-1.png)
