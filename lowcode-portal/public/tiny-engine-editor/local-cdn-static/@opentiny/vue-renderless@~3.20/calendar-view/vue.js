...
import { computedCalendar, handleEvents, selectDay, getEventByTime, isToday, dateIsToday, toToday, getAllWednesdaysInYear, getAllWednesdaysInMonth, getAllDatesOfCurrWeek, initWeeklyCalendar, getDatesOfPreviousWeek, getDatesOfNextWeek, getPrevWeek, getNextWeek, goPrevMonth, goNextMonth, currentDateChange, newSchedule, handleMouseenter, handleMouseleave, isSelectedDate, isStartOrEndDay, getDayBgColor, isShowNewSchedule, genDayTimes, isShowMark, computeCascaderOptions, handleCascaderChange, setCascaderVisible, getEventByMonth, getEventByDate, getWeekOfDate, handleDropBtnClick, getEventShowTime, getCurWeekEvent, touchstart, touchmove, touchend } from './index.js';
import throttle from './../common/deps/throttle.js';
const api = ["state", "renderProxy", "isToday", "dateIsToday", "getEventByTime", "selectDay", "toToday", "getPrevWeek", "getNextWeek", "goPrevMonth", "goNextMonth", "currentDateChange", "newSchedule", "handleMouseenter", "handleMouseleave", "isSelectedDate", "isStartOrEndDay", "getDayBgColor", "isShowNewSchedule", "isShowMark", "handleCascaderChange", "setCascaderVisible", "handleDropBtnClick", "getEventShowTime", "touchstart", "touchmove", "touchend"];
const initState = ({
  reactive,
  props,
  computed,
...
const initWatch = ({
  ...
  watch(() => state.eventsChangeHash, () => {
    if (state.mode === "schedule" || state.mode === "timeline") {
      api2.getCurWeekEvent();
    }
  });
  watch(() => state.activedWeekIndex, () => {
    nextTick(() => {
...
const initApi = ({
  ...
  Object.assign(api2, {
    ...
    getAllWednesdaysInYear: getAllWednesdaysInYear({
      state
    }),
    getAllWednesdaysInMonth: getAllWednesdaysInMonth({
      state
    }),
    getAllDatesOfCurrWeek: throttle(50, true, getAllDatesOfCurrWeek({
      props,
      state
    })),
    initWeeklyCalendar: initWeeklyCalendar({
      api: api2,
      state
    }),
    getDatesOfPreviousWeek: getDatesOfPreviousWeek({
      api: api2,
      state
    }),
    getDatesOfNextWeek: getDatesOfNextWeek({
      api: api2,
      state
    }),
    ...
    getPrevWeek: throttle(50, true, getPrevWeek({
      api: api2,
      state,
      emit
    })),
    getNextWeek: throttle(50, true, getNextWeek({
      api: api2,
      state,
      emit
    })),
    goPrevMonth: throttle(50, true, goPrevMonth({
      state
    })),
    goNextMonth: throttle(50, true, goNextMonth({
      state
    })),
    ...
    getDayBgColor: getDayBgColor({
      props
    }),
    isShowNewSchedule: isShowNewSchedule({
      props
    }),
    genDayTimes: genDayTimes({
      props
    }),
    ...
    getEventShowTime: getEventShowTime({
      state
    }),
    getCurWeekEvent: getCurWeekEvent({
      state,
      api: api2
    }),
    handleDropBtnClick: handleDropBtnClick({
      api: api2,
      state
    })
  });
