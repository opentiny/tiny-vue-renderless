...
const fontFamilyConfig = ["songti", "yahei", "kaiti", "heiti", "lishu", "mono", "arial", "arialblack", "comic", "impact", "times"];
const fontSizeConfig = ["12px", "13px", "14px", "15px", "16px", "17px", "18px", "19px", "20px", "22px", "24px", "26px", "29px", "32px", "36px", "40px", "48px", "72px"];
...
const defaultToolbar = [["undo", "redo", "clean"], [{
...
}], underline, ["blockquote", "code-block"], [{
  header: 1
}, {
  header: 2
}], list, script, [{
  ...
  align: []
}], ["link", "image"], ["better-table"], ["fullscreen"]];
const toolbar = () => {
  return {
...
const listAutofillHandler = ({
  ...
  const delta = new FluentEditor.imports["delta"]().retain(range.index - offset).delete(length + 1).retain(line.length() - 2 - offset).retain(1, {
    list: {
      value
    }
  });
...
const listAutofill = ({
  ...
  prefix: /^\s*?(\d+\.|-|\*|\[ ?\]|\[x\])$/,
  handler: listAutofillHandler({
    FluentEditor,
    state
  })
});
const listIgnoreRight = ({
  FluentEditor,
  state
}) => ({
  key: "ArrowRight",
...
const keyboard = ({
  ...
  return {
    bindings: __spreadProps(__spreadValues({}, BetterTable.keyboardBindings), {
      "list autofill": listAutofill({
        FluentEditor,
        state
      }),
      // fix:在list的offset为0时（即只在最左侧触发）加键盘事件，使其跳过contenteditable=false的span进行移动
      "list ignoreRight": listIgnoreRight({
        FluentEditor,
        state
      }),
      "list ignoreLeft": listIgnoreLeft({
        FluentEditor,
        state
      }),
      "list softBreak": listSoftBreak({
        FluentEditor,
        state
      })
    })
...
export { defaultOption, defaultToolbar, iconOption, iconOptionMobileFirst, keyboard, mention, simpleToolbar };