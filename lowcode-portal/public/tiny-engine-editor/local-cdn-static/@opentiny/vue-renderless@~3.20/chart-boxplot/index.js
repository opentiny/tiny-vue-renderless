...
import { getFormated, htmlHandler, getLegend } from './../chart-core/deps/utils.js';
...
const factoryFmt = ({
  ...
  i,
  digit
}) => val => getFormated(val, yAxisType[i], digit);
const getYAxis = args => {
  const {
    axisVisible,
    digit,
...
const boxplot = (columns, rows, settings, extra) => {
  ...
  const {
    lineStyle,
    areaStyle,
    tooltipLabel = ["lower", "Q1", "median", "Q3", "upper"]
  } = settings;
  const {
    legendVisible,
    outliers,
