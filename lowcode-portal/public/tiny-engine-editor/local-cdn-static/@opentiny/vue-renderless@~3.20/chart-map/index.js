...
import { getFormated, getMapJSON, getLegend as getLegendMap } from './../chart-core/deps/utils.js';
...
const getSeries = args => {
  ...
  metrics.forEach(itemName => {
    ...
    rows.forEach(row => itemResult.data.push({
      name: row[dimension],
      value: row[itemName],
      selected: selectData
    }));
    itemResult.emphasis = emphasis;
    itemResult.label = __spreadValues({
      color: "#191919"
    }, itemResult.label);
