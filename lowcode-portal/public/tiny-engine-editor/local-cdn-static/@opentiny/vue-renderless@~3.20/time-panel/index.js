...
const compareTime = api => (time1, time2) => {
...
const emitPick = ({
  ...
  const lastEmitValue = JSON.stringify({
    value,
    visible
  });
...
const handleClick = api => item => !item.disabled && api.emitPick({
  value: item.value
});
const handleClear = ({
  api,
...
export { compareTime, computItems, emitDestroy, emitPick, formatTime, handleClear, handleClick, handleKeydown, handleMenuEnter, isValidValue, nextTime, parseTime, scrollDown, scrollToOption, watchValue };