...
const formatDate = ({
  ...
  return format(date, dateFormat === void 0 ? defaultFormet : dateFormat);
};
...
const initPanel = ({
  ...
  const dateList = {};
  Array.from({
    length: 12 * state.yearNum
  }).forEach(() => {
    const startWeek = date.getDay();
    dateList[getDateStr(year, month)] = Array.from({
      length: startWeek
    }).map(() => ({}));
...
const watchVisible = ({
  ...
  if (bool) {
    ...
    setTimeout(() => api.scrollToCurrentDate({
      date: currentDate
    }), 300);
  }
...
const watchModelValue = ({
  ...
  if ([DATE_TIME_RANGE, DATE_TIME].includes(type)) {
    ...
    Array.from({
      ...
      if (date) {
        state.timeList[index] = [getFormatTime(date.getHours()), getFormatTime(date.getMinutes()), getFormatTime(date.getSeconds())];
      }
    });
  }
};
...
export { clear, close, confirm, formatDate, getCurrentDate, getDate, getDateStr, getDaysByWeek, getSelectedPosition, getWeeksByMonth, initPanel, loadingDate, scrollEnd, scrollStart, scrollToCurrentDate, selectOption, selectedComputed, timeConfirm, timeToggle, watchModelValue, watchVisible };