...
import { getDateStr } from './index.js';
const initYearMonthPanel = ({
  state,
  props
}) => ({
  dateValue,
  ...
  Array.from({
    length: state.yearNum
  }).map((item, index) => {
    const currentYear = year + index + 1;
    years[currentYear] = Array.from({
      length: 12
    }).map((v, index2) => {
...
export { getOffsetMonth, initYearMonthPanel, loadYearMonth };