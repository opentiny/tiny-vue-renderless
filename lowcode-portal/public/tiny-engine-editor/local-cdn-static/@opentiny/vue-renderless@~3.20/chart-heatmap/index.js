...
import { getFormated, getBmap, getAmap, getMapJSON } from './../chart-core/deps/utils.js';
import { HEAT_MAP_COLOR, HEAT_DEFAULT_COLOR, itemPoint, itemLabel, itemContent } from './../chart-core/deps/constants.js';
...
const getResult = args => {
  ...
  if (type === "bmap") {
    ...
    if (jsonStr) {
      const geoAttr = __spreadValues({
        map: jsonStr
      }, geo);
      beforeRegisterMap && (jsonStr = beforeRegisterMap(mapOrigin));
      echarts.registerMap(jsonStr, {
        geoJSON: mapOrigin
      }, specialAreas);
      res = new Promise(resolve => resolve(__spreadValues({
        geo: geoAttr
      }, options)));
    } else {
      let params = {
    ...
    res = getAmap(key, v).then(() => __spreadValues({
      amap
    }, options));
  } else {
    res = __spreadValues({
...
const heatmap = (columns, rows, settings, extra, extend) => {
  ...
  let {
    xAxisList: innerXAxisList,
    yAxisList: innerYAxisList
  } = settings;
  let {
    chartData = [],
    extraMetrics = [],
    mainColumn = dimension.concat([metrics])
  } = {};
