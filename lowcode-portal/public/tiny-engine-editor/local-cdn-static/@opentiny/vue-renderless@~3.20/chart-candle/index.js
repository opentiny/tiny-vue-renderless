    ...
    options.forEach(opt => {
      ...
      if (componentSubType === "candlestick") {
        ...
        metrics.slice(0, 4).forEach((m, i) => {
          const name2 = isNull(labelMap[m]) ? m : labelMap[m];
          const value = getFormated(data[i + 1], dataType, digit);
          tplt.push(`${itemLabel(`- ${name2}`)}${itemContent(value)}<br>`);
        });
...
const candle = (columns, rows, settings, extra) => {
  const {
    dimension = columns[0],
  ...
  const {
    showDataZoom = false,
    downColor = DEF_DOWN_COLOR,
    upColor = DEF_UP_COLOR
  } = settings;
  const {
    start = DEF_START,
    end = DEF_END,
  ...
  const grid = getCandleGrid({
    showVol
  });
  const xAxis = getCandleXAxis({
    dims
  });
