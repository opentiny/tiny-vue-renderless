...
import { xss, log } from './../common/xss.js';
import uploadAjax from './../common/deps/upload-ajax.js';
...
const noopFnCreator = (fn, propName) => {
  const noFn = () => {
    if (propName) {
      return Promise.reject(new Error(`[TINY Error][FileUpload] Prop ${propName} is mandatory when the framework service is not used`));
    } else {
      return Promise.reject(new Error("[TINY Error][FileUpload] Prop action is mandatory when the framework service is not used"));
    }
  };
  return fn || noFn;
};
...
const computedUploadDisabled = ({
  props,
  state
}) => () => props.disabled || (state.form || {}).disabled;
const computedUploadingSize = ({
  state,
  constants
...
const watchListType = ({
  ...
  if ([constants.LIST_TYPE.PICTURE_CARD, constants.LIST_TYPE.PICTURE, constants.LIST_TYPE.PICTURE_SINGLE, constants.LIST_TYPE.DRAG_SINGLE].includes(type)) {
    state.uploadFiles = state.uploadFiles.map(file => {
      file.type = api.getFileSourceType({
        file
      });
...
const beforeUpload = ({
  ...
  if (file) {
    ...
    if (!isValid) {
      ...
      return Modal.message({
        message: fileType ? t(constants.EDM.notSupport, {
          format: fileType
  ...
  if (flag) {
    const {
      rawFile = file.raw,
      before = props.beforeUpload(rawFile)
    } = {};
...
const properFileSize = ({
  ...
  if ([void 0, null].includes(file.size)) return true;
  let maxSize = 0;
  ...
  if (file.size <= userMin * 1024) {
    Modal.message({
      message: `${t(constants.EDM.SIZE, {
        minSize: api.formatFileSize(Number(userMin), "K"),
        sizeUnit: ""
      })}`,
      status: "warning"
    });
...
const addFileToList = ({
  ...
  if ([constants.LIST_TYPE.PICTURE_CARD, constants.LIST_TYPE.PICTURE, constants.LIST_TYPE.PICTURE_SINGLE, constants.LIST_TYPE.DRAG_SINGLE].includes(props.listType)) {
    try {
      if (state.isHwh5) {
...
const handleHwh5Files = (files, hwh5) => {
  ...
  return files.map(file => {
    ...
    const updateFile2 = __spreadProps(__spreadValues({}, f), {
      type,
      name,
      filePath,
...
const handleSuccess = ({
  ...
  if (Array.isArray(rawFile)) {
    ...
    const {
      STATUS_SPECIAL_CHARACTERS,
      NOT_SUPPORT_SPECIAL_CHARACTERS
    } = constants.EDM;
...
const handleRemove = ({
  ...
  let doRemove = () => {
    ...
    emit("remove", __spreadProps(__spreadValues({}, file), {
      status: constants.FILE_STATUS.FAIL
    }), fileList);
  };
...
const abortDownload = ({
  ...
  if (Array.isArray(file)) {
    if (batch) {
      cancel(file.map(f => f.docId || f).sort().join(","));
    } else {
...
const getTranslateFile = ({
  ...
  if (isChunk) {
    if (index === 0) {
      state.downloadCancelData[file.docId] = api.setWriterFile({
        data,
        index,
        isLessThan17G,
        file
...
const validateDownloadStatus = ({
  ...
  const errorHandle = ({
    ...
    errRes,
    Modal: Modal2,
    downloadOps: downloadOps2
  }) => {
    if (state2.currentDownloadFiles && state2.currentDownloadFiles.docId === file2.docId) return;
...
const downloadFileSingle = ({
  ...
  if (fileUrl) {
    ...
    promise = service.getSingleDownloadUrl().then(url => {
      ...
      serviceUrl = api.modifyServiceUrlSingle({
        file,
        serviceUrl,
        range
      });
  ...
  promise.then(url => {
    ...
    service.get(url, params).then(data => {
      if (api.getKiaScanTip({
        data
      })) return;
      if (api.validateDownloadStatus({
        downloadOps: props.edm.download || {},
        file,
        isLessThan17G,
        data
      })) return;
...
const downloadFileBatch = ({
  ...
  api.getToken(tokenParams).then(data => {
    ...
    service.getPackageDownloadUrl().then(url => {
      ...
      service.post(url, __spreadValues(__spreadValues({}, params), range), {
        withCredentials: props.withCredentials,
        headers: Object.assign(props.headers, state.headers),
        responseType: "blob",
        cancelToken: api.createDownloadCancelToken(file),
  ...
  return service.getAsyncPackageDownload().then(url => {
    service.request({
      ...
      if (res && res.data && res.data.status === 200) {
        const files = (res.data.result || []).map(({
          downloadLink,
          fileSize
        }) => {
          return {
        ...
        files.forEach(file => {
          ...
          const translateFile = getTranslateFile({
            api,
            isChunk,
            isLessThan17G,
            file,
...
const downloadFile = ({
  ...
  if (!state.isEdm) {
    ...
    if (state.isHwh5) {
      ...
      files.forEach(f => api.downloadFileSingleInner({
        file: f,
        isBatch: false
      }));
      return;
...
const downloadFileSingleInner = ({
  ...
  api.getToken(tokenParams).then(data => {
    if (!data) return;
    if (state.isHwh5) {
      api.downloadFileSingleHwh5({
        file
      });
      return;
    }
    const promise = state.hasFileInfoInterface ? api.getDownloadFileInfo({
      docId: file.docId
    }) : Promise.resolve();
    promise.then(fileInfo => {
      ...
      if (fileSize && isLargeFile) {
        api.largeDocumentDownload({
          file: fileInfo,
          isBatch,
          isLessThan17G: fileSize < SIZE_17G * 1024
        });
      } else {
        api.downloadFileInner({
          file,
  ...
  return service.getDocumentInfoUrl().then(url => {
    return new Promise((resolve, reject) => {
      service.request({
        ...
        withCredentials: props.withCredentials,
        headers: Object.assign(props.headers, state.headers),
        cancelToken: api.createDownloadCancelToken({
          docId
        }),
        data: {
          docInfoVO: {
...
const setWriterFile = ({
  ...
  let {
    ...
    fileSize,
    docName
  } = state.downloadChunkFile[file.docId];
  if (content) {
    docName = content.match(/fileName.?=(.*)/)[1] || content.match(/fileName=(.*)/)[1] || docName;
  }
  if (!isLessThan17G) {
    fileStream = Streamsaver.createWriteStream(docName, {
    ...
    if (progress.file.cancelToken) {
      ...
      service.request({
        ...
        if (data.response && state.errorStatusCodes.includes(data.response.status)) {
          ...
          api.getToken(tokenParams).then(data2 => afterUpload({
            data: data2,
            file,
            batchIndex,
            state,
...
const segmentUpload = ({
  ...
  if (typeof file.then === "function") {
    file.then(file2 => new Promise(resolve => {
      if (props.edm.isCheckCode !== true) return resolve(file2);
      const reader = new FileReader();
    ...
    try {
      ...
      api.getToken(tokenParams).then(data => {
        ...
        if (isObject(file) || Array.isArray(file) && file.length === 1) {
          api.previewFileSingle({
            file: Array.isArray(file) ? __spreadValues({}, file[0]) : file,
            resolve,
            open
          });
        } else if (Array.isArray(file) && file.length > 1) {
          api.previewFileBatch({
            file,
            resolve,
            open
          });
...
const getNewTabPreviewUrl = ({
  api
}) => file => api.previewFile(file, true);
const previewFileSingle = ({
  api,
  state,
  props,
  constants,
  ...
  service.all([service.getPreviewUrl(), service.getSingleDownloadUrl()]).then(service.spread((previewUrl, downloadurl) => {
...
const previewFileBatch = ({
  ...
  service.getPreviewUrlBatch().then(url => {
    ...
    service.request({
      ...
      const iframeUrl = api.getPreviewUrlSync({
        generate: data.result.generate,
        size: file.length
      }, true);
      if (open) return resolve(iframeUrl);
      if (iframeUrl) {
  ...
  if (batch) {
    ...
    if (webPreview) {
      ...
      paramsData = Object.assign({}, watermark, {
        "EDM-Authorization": edmToken,
        lang,
        bar: window.btoa(JSON.stringify(__spreadProps(__spreadValues({}, bar), {
    ...
    try {
      api.getToken({
        ...
        service.getPreviewUrl().then(url => {
          if (isObject(file) || Array.isArray(file) && file.length === 1) {
            api.previewImageSingle({
              url,
              file: Array.isArray(file) ? __spreadValues({}, file[0]) : file
            }).then(link => resolve(link));
          } else if (Array.isArray(file) && file.length > 1) {
            api.previewImageBatch({
              url,
              file
...
const previewImageBatch = ({
  ...
  const promises = [];
  file.forEach(f => {
    promises.push(api.previewImageSingle({
      url,
      file: f
    }));
  });
...
const handleChange = ({
  ...
  if (typeof file === "object" && file !== null && file !== void 0) {
    ...
    vm.$refs[constants.UPLOAD_INNER].$refs[constants.UPLOAD_INNER_TEMPLATE].handleChange({
      target: {
        files
      }
    });
...
const getTipMessage = ({
  ...
  let limitTip = limit ? t(constants.NUMBER_LIMIT, {
    number: limit
  }) : "";
...
export { abort, abortDownload, addFileToList, afterDownload, batchSegmentDownload, batchSegmentUpload, beforeUpload, calcUploadingFilesInfo, clearFiles, clearUploadingFiles, closeRecordPanel, computeDocChunkSize, computedSourcetype, computedUploadDisabled, computedUploadingSize, createDownloadCancelToken, downloadAsyncPackage, downloadFile, downloadFileBatch, downloadFileInner, downloadFileSingle, downloadFileSingleHwh5, downloadFileSingleInner, encryptDialogConfirm, getCalcProgress, getDialogConfigObj, getDownloadFileInfo, getFile, getFileHash, getFileSourceType, getFileUploadUrl, getFormData, getHandleSuccess, getKiaScanTip, getNewTabPreviewUrl, getPreviewUrlSync, getTipMessage, getToken, handleChange, handleClick, handleClickFileList, handleError, handleFileClick, handleProgress, handleReUpload, handleReUploadTotal, handleRemove, handleStart, handleSuccess, handleTriggerClick, initService, largeDocumentDownload, largeDocumentUpload, modifyServiceUrlSingle, mounted, noopFnCreator, onBeforeDestroy, previewFile, previewFileBatch, previewFileSingle, previewImage, previewImageBatch, previewImageSingle, properFileSize, segmentUpload, segmentUploadInit, setWriterFile, sliceChunk, sliceDownloadChunk, startUpload, submit, updateFile, updateUrl, validateDownloadStatus, watchFileList, watchListType };