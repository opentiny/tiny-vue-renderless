...
import { formatDate } from './../common/deps/date-util.js';
import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, prevDate, nextDate, isDate, clearTime } from './../common/deps/date-util.js';
  ...
  for (let i = 0; i < 6; i++) {
    ...
    for (let j = 0; j < 7; j++) {
      const {
        cell,
        cellDate
      } = api.getCell(row, i, j, DATEPICKER.Normal, props);
      count = doCount({
        i,
        day,
        offset,
    ...
    if (props.selectionMode === DATEPICKER.Week) {
      ...
      const isWeekActive2 = api.isWeekActive(row[start + 1]);
      Object.assign(row[start], {
        inRange: isWeekActive2,
        start: isWeekActive2
      });
      Object.assign(row[end], {
        inRange: isWeekActive2,
        end: isWeekActive2
      });
    }
...
const getCellClasses = ({
  ...
  if (selectionMode === DATEPICKER.Day && ~[DATEPICKER.Normal, DATEPICKER.Today].indexOf(cell.type) && api.cellMatchesDate(cell, props.value || state.date)) {
...
export { arrayFind, arrayFindIndex, cellMatchesDate, coerceTruthyValueToArray, getCell, getCellClasses, getCellRangeClasses, getCssToken, getDateOfCell, getDateTimestamp, getOffsetDay, getRows, getWeeks, handleClick, handleMouseMove, isWeekActive, markRange, removeFromArray, watchMaxDate, watchMinDate };