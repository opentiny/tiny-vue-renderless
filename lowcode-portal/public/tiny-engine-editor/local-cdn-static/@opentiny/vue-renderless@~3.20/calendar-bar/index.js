...
import { lastMonth, nextMonth } from './../common/calendar/calendar.js';
...
const normalConfig = (config, state) => {
  ...
  const weekFirst = getWeekFirst(state, {
    config
  });
...
const getDate = (state, type, date) => {
  ...
  if (type === "last") {
    const {
      year,
      month
    } = lastMonth(state.currentYear, state.currentMonth);
    resDate = /* @__PURE__ */new Date(`${year}-${month}-01`);
  } else if (type === "next") {
    const {
      year,
      month
    } = nextMonth(state.currentYear, state.currentMonth);
    resDate = /* @__PURE__ */new Date(`${year}-${month}-01`);
  } else {
...
const getCalendarDays = state => (date, config, type = "cur") => {
  ...
  const calendarDays = Array.from({
    length: monthDays[currentMonth]
  }).map((v, i) => buildDay(currentFullYear, currentMonth, i + 1));
  const weekDays = Array.from({
    length: dayOfWeek
  }).map((v, i) => (weekFirst + i) % dayOfWeek);
  const monthDaysFirst = new Date(f(calendarDays).dateStr).getDay();
  const monthDaysLast = new Date(l(calendarDays).dateStr).getDay();
  const padCalendarDays = getPadCalendarDays({
    calendarDays,
    buildDay
  });
...
function getDateStr(state, year, month) {
  ...
  const {
    currentFullYear: curYear,
    currentMonth: curMonth,
    currentDate: curDate
  } = splitDate(new Date(dateStr));
