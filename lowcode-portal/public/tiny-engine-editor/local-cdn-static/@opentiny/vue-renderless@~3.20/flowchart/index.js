...
import { omitText as omit } from './../common/string.js';
...
const compute = ({
  ...
  const seg = (qty, segs) => {
    ...
    return Array.from({
      length: segs
    }).map((o, i) => quot + (i === 0 ? rmd1 : i === segs - 1 ? rmd2 : 0));
  };
...
const buildAfterLink = () => (links, widths, heights, afterNodes) => {
  const path = (link, cWidth, rHeight, afterNodes2) => {
    ...
    if (p) {
      return {
        type: "link",
        p: relative(fx, fy, p, cWidth, rHeight),
        raw: link
      };
    }
    return {
...
const computeMf = ({
  ...
  const {
    ...
    links,
    groups
  } = hideNodeLink(props.data);
  const getRow = buildGetRow({
    afterConfig,
    api
  });
  const getCol = buildGetCol({
    afterConfig,
    api
  });
  const rectRow = buildRectRow({
    afterConfig,
    getRow
  });
  const rectNode = buildRectNode({
    afterConfig,
    api,
    getRow,
    rectRow
  });
  const buildAfterNodeGraphArgs = {
    afterConfig,
    api,
    getCol,
    getRow,
  ...
  const {
    ...
    afterNodes,
    graph
  } = buildAfterNodeGraph(buildAfterNodeGraphArgs);
  const buildAfterLinkArrowArgs = {
    afterConfig,
    afterNodes,
    graph,
    links
  ...
  const {
    afterLinks,
    arrows
  } = buildAfterLinkArrow(buildAfterLinkArrowArgs);
  const hoverState = api.buildHoverState({
    afterGroups,
    afterLinks
  });
...
const buildRectRow = ({
  ...
  const accrueRowHeight = Array.from({
    length: row
  }).map((c, i) => i).map(i => getRow(afterNodes, i, "height")).reduce((p, c) => p + c, 0);
...
const buildRectNode = ({
  ...
  const accrueColWidth = Array.from({
    length: col
  }).map((c, i) => i).map(i => {
...
const customLayout = ({
  ...
  if (typeof afterConfig.layout === "function") {
    ...
    const coords = afterConfig.layout({
      afterNodes,
      graphHeight,
      graphWidth
    });
...
const buildAfterNodeGraph = args => {
  ...
  const afterGroups = computeGroup({
    afterConfig,
    afterNodes,
    graph,
    groups
...
const computeGroup = ({
  ...
  if (groups && groups.length) {
    return groups.map(group => {
      ...
      const coords = computeGroupCoord({
        afterNodes,
        group
      });
      const padding = [0, 0];
...
const computeGroupCoord = ({
  afterNodes,
  group
}) => {
  const points = [];
  const coords = ["", "", "", ""];
  group.nodes.map(name => afterNodes.find(afterNode => afterNode.raw.name === name)).forEach(({
    x,
    y,
    width,
    height
...
const getFromToRect = ({
  ...
  const afterNodeCouple = [afterLink.raw.from, afterLink.raw.to].map(name => afterNodes.find(afterNode => afterNode.raw.name === name));
  const {
    x: x0,
    y: y0,
    width: w0,
  ...
  if (Array.isArray(linkPath)) {
    for (let i = 0; i < linkPath.length; i++) {
      if (hitMatch(linkPath[i], afterLink)) {
        const fromToRect = getFromToRect({
          afterLink,
          afterNodes
        });
        const {
          ...
          linear,
          flag
        } = validLinkPath(linkPath[i].method(__spreadProps(__spreadValues({
          afterLink,
          afterNodes
        }, fromToRect), {
          api: linkPathApis
    ...
    if (hitMatch(linkPath, afterLink)) {
      ...
      const {
        ...
        linear,
        flag
      } = validLinkPath(linkPath.method(__spreadProps(__spreadValues({
        afterLink,
        afterNodes
      }, fromToRect), {
        api: linkPathApis
...
const buildAfterLinkArrow = args => {
  ...
  const arrows = [];
  const buildArrow = getBuildArrow({
    arrows,
    afterConfig
  });
  const adjust = adjustLine(afterConfig);
...
const setAdjustY = ({
  ...
  if (!lastRowAfterNodes.length) {
    lastRowAfterNodes = state.temporary.lastRowAfterNodes = state.afterData.afterNodes.filter(afterNode => afterNode.lastRow);
  }
...
export { addListeners, antialiasing, buildAfterLink, buildAfterNode, buildHoverState, clearDropdown, clearHoverAfterLink, clickGroup, clickNode, compute, computeMf, computedIcon, drawAfterLink, getAllItem, getNode, getNodeDef, getVars, handleNodeResize, hitTest, initDropdowns, isLinkHover, isMf, omitText, refresh, removeListeners, runAdjustYTask, setAdjustY, setListeners, setNodeAdjustY };