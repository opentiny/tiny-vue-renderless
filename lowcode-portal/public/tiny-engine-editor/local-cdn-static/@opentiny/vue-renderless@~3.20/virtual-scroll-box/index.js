...
import getScrollbarSize from './../common/deps/scrollbar-width.js';
...
const preprocessVirtualScrollSlice = ({
  items,
  itemSizes,
  fixedItems
}) => {
...
const createVirtualScroll = ({
  ...
  const xAxisStyle = {
    width: `${columnTotal}px`,
    height: "1px"
  };
  const yAxisStyle = {
    height: `${rowTotal - 1}px`,
    width: "1px"
  };
  const viewBaseStyle = {
    width: "100%",
  ...
  } = preprocessVirtualScrollSlice({
    items: columns,
    itemSizes: columnSizes,
...
const refresh = ({
  ...
  const {
    isKeepScrollTop = false,
    isKeepScrollLeft = false
  } = options || {};
...
export { analyzeScrolling, computedBarSize, createVirtualScroll, onScroll, refresh };