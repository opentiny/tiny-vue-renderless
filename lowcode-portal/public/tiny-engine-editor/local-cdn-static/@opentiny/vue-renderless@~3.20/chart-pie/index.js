...
import { getFormated, setArrayValue, cloneDeep } from './../chart-core/deps/utils.js';
...
const getPieSeries = args => {
  ...
  rowsTemp.forEach((dataRows, index) => {
    ...
    serie.label = __spreadValues({
      color: "#191919"
    }, label);
    serie.labelLine = __spreadValues({
      smooth: true,
      lineStyle: {
        color: "#bbb"
      }
    }, labelLine);
    serie.itemStyle = __spreadValues({
      borderWidth: 2,
      borderColor: "#fff"
    }, itemStyle);
    if (percentShow) {
...
const pie = (columns, rows, settings, extra, isRing) => {
  ...
  const pieTooltip = getPieTooltip({
    dataType,
    innerRows,
    limitShowNum,
    digit,
