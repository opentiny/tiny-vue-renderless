...
import { getFormated, getStackMap, get, set, cloneDeep } from './../chart-core/deps/utils.js';
...
const factoryFmt = ({
  ...
  i,
  digit
}) => val => getFormated(val, meaAxisType[i], digit);
const getBarMeaAxis = args => {
  ...
  for (; i < 2; i++) {
    if (meaAxisType[i]) {
      ...
      meaAxis[i] = __spreadProps(__spreadValues({}, meaAxisBase), {
        axisLabel: {
          formatter
        }
      });
...
const getBarSeries = args => {
  ...
  series = Object.keys(seriesTemp).map(item => {
    ...
    if (Object.keys(stack).length) {
      ...
      seriesItem.itemStyle = __spreadValues({
        borderWidth: 2,
...
const bar = (columns, rows, settings, extra) => {
  ...
  let legend = legendVisible && getLegend({
    metrics,
    labelMap,
    legendName
  });
  let yAxis = getBarDimAxis({
    innerRows,
    dimAxisName,
    dimension,
    axisVisible,
  ...
  let tooltip = tooltipVisible && getBarTooltip({
    axisSite,
    isHistogram,
    meaAxisType,
    digit,
