...
const initApi = ({
  ...
  Object.assign(api2, {
    ...
    onMouseenterSelf: onMouseenterSelf({
      state
    }),
    onCopying: onCopying({
      state,
    ...
    computedDisabledTooltipContent: computedDisabledTooltipContent({
      props,
      state
    }),
    ...
    computedCurrentSizeMap: computedCurrentSizeMap({
      state,
      designConfig
    })
  });
...
const addWatch = ({
  ...
  if (props.renderType === "grid" && !props.optimization) {
    watch(() => state.gridData, api2.setSelected, {
      immediate: true
    });
  }
  if (props.renderType === "tree" && !props.optimization) {
    watch(() => state.treeData, api2.setSelected, {
      immediate: true
    });
  }
