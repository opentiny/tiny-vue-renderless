...
import { getNodeKey as innerGetNodekey } from './../common/deps/tree-model/util.js';
...
const computedShowEmptyText = ({
  constants,
  t
}) => props => props.emptyText || t(constants.EMPTY_TEXT_LOCAL);
const computedIsEmpty = () => (props, state) => {
  ...
  return !childNodes || childNodes.length === 0 || childNodes.every(({
    visible
  }) => !visible) || props.viewType === "plain" && state.plainNodeStore.filteredNodes.length === 0;
};
const watchDefaultCheckedKeys = state => value => {
  state.store.setDefaultCheckedKey(value);
};
const watchDefaultExpandedKeys = ({
  state,
  api,
  nextTick
}) => value => {
...
const deleteConfirm = ({
  ...
  if (typeof props.deleteNodeMethod === "function") {
    const promise = props.deleteNodeMethod({
      event: event2,
      node
    });
...
const setAddDisabledKeys = ({
  state
}) => data => {
  state.action.addDisabled = data || [];
};
const setEditDisabledKeys = ({
  state
}) => data => {
  state.action.editDisabled = data || [];
};
const setDeleteDisabledKeys = ({
  state
}) => data => {
  state.action.deleteDisabled = data || [];
};
const initPlainNodeStore = ({
  props,
  state
}) => () => {
  const {
  ...
  plainNodeStore.filteredNodes = plainNodeStore.nodes.filter(plainNode => plainNode.node.visible && filterPlainNode(plainNode.node.data));
  state.plainNodeStore = plainNodeStore;
};
const handleCheckPlainNode = ({
  props,
  emit
}) => (e, plainNode) => {
...
export { addNode, append, beforeUnmount, cancelDelete, clearCurrentStore, closeEdit, closeMenu, computedIsEmpty, computedShowEmptyText, computedTreeItemArray, created, deleteAction, deleteConfirm, dragEnd, dragOver, dragStart, editNode, expandAllNodes, filter, getCheckedKeys, getCheckedNodes, getChildren, getCurrentKey, getCurrentNode, getHalfCheckedKeys, getHalfCheckedNodes, getNode, getNodeKey, getNodePath, getSameDataIndex, handleCheckPlainNode, handleClickPlainNode, handleKeydown, handleNodeExpand, initIsCurrent, initPlainNodeStore, initTabIndex, initTreeStore, insertAfter, insertBefore, loopGetTreeData, mounted, openEdit, remove, saveEdit, saveNode, setAddDisabledKeys, setChecked, setCheckedByNodeKey, setCheckedKeys, setCheckedNodes, setChildren, setCurrentKey, setCurrentNode, setCurrentRadio, setDeleteDisabledKeys, setEditDisabledKeys, switchToggle, updateKeyChildren, updated, watchCheckStrictly, watchCheckboxItems, watchData, watchDefaultCheckedKeys, watchDefaultExpandedKeys, wrapMounted };