...
import { toDateStr } from './../common/date.js';
import { toJsonStr } from './../common/object.js';
import { toJson } from './../common/string.js';
import { log } from './../common/xss.js';
...
const request = {
  ...
  singleRequest(param, api, cb) {
    ...
    const params = valueField === "userId" ? queryStr : {
      valueField,
      valueSplit,
      [valueField]: queryStr
    };
...
const syncCacheIds = ({
  ...
  ids.forEach(id => {
    ...
    if (cacheUser2) {
      ...
      cacheData.push(Object.assign(user, {
...
const visibleChange = ({
  ...
  if (!show) {
    state.visible = show;
    state.options = state.selected.map(user => Object.assign(user, {
      _show: true
    }));
  }
...
const initService = ({
  ...
  const noopFnCreator = propName => () => {
    if (propName) {
      return Promise.reject(new Error(`[TINY Error][User]] Prop ${propName} is mandatory when the framework service is not used`));
    } else {
      return Promise.reject(new Error("[TINY Error][User]] Prop service is mandatory when the framework service is not used"));
    }
  };
...
const filter = ({
  ...
  if (props.multiple && props.hideSelected) {
    ...
    return state.options.map(user => {
      const _show = !~selectedUsers.indexOf(typeof user[state.valueField] === "string" ? user[state.valueField].toLocaleLowerCase() : user[state.valueField]);
...
const computedTextField = ({
  service,
  props
}) => () => props.textField || service.textField || "userCN";
const computedValueField = ({
  service,
  props
}) => () => props.valueField || service.valueField || "userId";
export { autoSelect, cacheUser, computedTextField, computedValueField, filter, getUsers, handleBlur, initService, initUser, saveCache, searchMethod, setSelected, suggestUser, syncCacheIds, updateCache, updateOptions, useSortable, userChange, visibleChange };