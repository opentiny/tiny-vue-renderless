...
import { hasOwn, isFunction } from './../type.js';
...
Schema.prototype = {
  ...
  validate(source_, o = {}, oc = () => void 0) {
    ...
    const errorFields = {};
    return asyncMap(seriesData, options, (data, doIt) => {
      ...
      if (validResult && validResult.then) {
        validResult.then(() => validHandler(), e => validHandler(e));
      }
    }, results => {
      complete(results);
    });
