...
import { isNull, isDate } from './../type.js';
const fecha = {};
...
const parseFlags = {
  d: [twoDigits, (date, value) => {
    date.day = value;
  }],
  Do: [twoDigits + word, (date, value) => {
    date.day = parseInt(value, 10);
  }],
  M: [twoDigits, (date, value) => {
    date.month = value - 1;
  }],
  yy: [twoDigits, (date, value) => {
    const now = /* @__PURE__ */new Date();
    const cent = Number(String(now.getFullYear()).substr(0, 2));
    date.year = String(value > 68 ? cent - 1 : cent) + value;
  }],
  h: [twoDigits, (date, value) => {
    date.hour = value;
  }],
  m: [twoDigits, (date, value) => {
    date.minute = value;
  }],
  s: [twoDigits, (date, value) => {
    date.second = value;
  }],
  yyyy: [fourDigits, (date, value) => {
    date.year = value;
  }],
  S: ["\\d", (date, value) => {
