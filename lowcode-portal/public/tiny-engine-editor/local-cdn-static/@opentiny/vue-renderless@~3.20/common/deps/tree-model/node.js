...
import { markNodeData, NODE_KEY } from './util.js';
import { indexOf } from './../../array.js';
import { hasOwn, typeOf } from './../../type.js';
...
class Node {
  constructor(options) {
    ...
    if (key && currentNodeKey !== void 0 && this.key === currentNodeKey) {
  ...
  updateChildren() {
    ...
    children.forEach((item, index) => {
      ...
      const isNodeExists = !!key && indexOf(oldChildren, key, (item2, data) => item2[NODE_KEY] === data) >= 0;
      if (isNodeExists) {
        newChildrenMap[key] = {
          index,
          data: item
  ...
  loadData(callback, defaultProps = {}) {
    ...
    if (lazy === true && load && !this.loaded && (!this.loading || Object.keys(defaultProps).length)) {
      ...
      this.store.load(this, children => {
        ...
        typeof this.store.afterLoad === "function" && this.store.afterLoad({
          data: children
        });
      });
    } else {
