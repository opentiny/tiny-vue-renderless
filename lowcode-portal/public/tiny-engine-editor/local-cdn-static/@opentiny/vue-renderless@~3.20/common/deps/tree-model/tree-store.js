...
import { getNodeKey } from './util.js';
...
class TreeStore {
  constructor(options) {
    ...
    if (this.lazy && this.load) {
      this.load(this.root, data => {
        ...
        typeof this.afterLoad === "function" && this.afterLoad({
          data,
          init: true
        });
      });
  ...
  append(data, parentData, index) {
    ...
    if (parentNode) {
      const child = parentNode.insertChild({
        data
      }, index);
  ...
  getAllData() {
    ...
    const walkTree = nodes => {
      return nodes.map(node => {
        return __spreadProps(__spreadValues({}, node.data), {
          [children]: walkTree(node.childNodes)
        });
      });
    };
