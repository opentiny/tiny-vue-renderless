...
import { arrayEach, findIndexOf, find } from './../grid/static/index.js';
  ...
  if (popseletor === "grid") {
    ...
    if (columns.length) {
      ...
      gridColumns = [{
        ...
        field: valueField,
        title: String(valueField).toUpperCase()
      }, {
        field: textField,
        title: String(textField).toUpperCase()
      }];
    }
  }
  return gridColumns;
...
const selectedBoxDelete = ({
  ...
  if (multi && popseletor === "grid") {
    vm.$refs.multiGrid.setSelection(find(vm.$refs.multiGrid.$refs.tinyTable.selection, item => item[props.valueField] === row[props.valueField]), false);
  }
  if (multi && popseletor === "tree") {
...
const getTreeSelect = ({
  ...
  arrayEach(checkedNodes, item => {
    ...
    const tmp = {
      _$title: plainNode.title,
      _$auxi: plainNode.auxi
    };
...
const multiTreeCheck = ({
  ...
  if (popseletor === "tree") {
    nextTick(() => {
      const datas = multi ? getTreeSelect({
        vm,
        props
      }) : getTreeRadio(vm);
      const values = datas.map(item => item[props.valueField]);
...
const computedConfig = ({
  ...
  if (popseletor === "grid") {
    if (multi && type === "select") {
      config = Object.assign(config, selectConfig, {
        checkRowKeys: selectedValues
      });
    }
    if (!multi && type === "radio") {
      config = Object.assign(config, radioConfig, {
        checkRowKey: selectedValues[0]
      });
    }
  }
  return config;
...
export { clearStatus, computedConfig, computedGridColumns, doAutoLookup, doMultiTreeFilter, emitChange, emitClose, getSelection, multiGridRadioChange, multiGridSelectAll, multiGridSelectChange, multiTreeAfterLoad, multiTreeCheck, multiTreeFilterNode, multiTreeFilterPlain, multiTreeLeavePlain, multiTreeRadio, onFooterCancel, onFooterConfirm, queryGridData, selectedBoxClear, selectedBoxDelete, selectedBoxDrag, selectedBoxInit, setChecked, setGridSelection, setSelection, setTreeSelection, watchMulti };