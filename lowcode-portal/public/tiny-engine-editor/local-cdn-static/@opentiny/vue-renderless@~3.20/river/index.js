...
import { layout, bpath, txt, hexToRgba } from './river.js';
const buildOption = ({
  api,
  emit,
  props,
  ...
  const chartData = layout(props.data, __spreadProps(__spreadValues({}, props.config), {
    canvasWidth: WIDTH
  }), vm.$el);
  ...
  const renderIcon = api.getRenderIcon({
    chartIconData,
    STATUS_COLOR,
    STATUS_YES,
    STATUS_NO
...
const getRenderNode = ({
  ...
  return (params, api) => {
    ...
    const lg = g[0].map((stop, i) => ({
      offset: stop,
      color: so(dataNode, g[1][i])
    }));
    ...
    return {
      ...
      children: [{
        ...
        shape: rectShape,
        style: {
          fill: new echarts.graphic.LinearGradient(0, 0, 1, 0, lg)
        }
      }, {
        type: "text",
        style: {
    ...
    return {
      ...
      children: [{
        ...
        shape: {
          cx: 0,
          cy: 0,
          r: si(dataIcon, "radius", radius)
        },
        style: {
          fill: so(dataIcon, hexToRgba(si(dataIcon, "fill", circleFill)))
        }
      }, {
        type: "text",
        style: {
...
const setListeners = ({
  ...
  state.myChart.on("click", {
    seriesName: "1"
  }, params => emit("click-node", params));
  state.myChart.on("click", {
    seriesName: "2"
  }, params => emit("click-link", params));
  state.myChart.on("click", {
    seriesName: "3"
  }, params => emit("click-icon", params));
  state.myChart.on("click", {
    seriesName: "4"
  }, params => emit("click-lege", params));
};
const setRiverRgbaOpacity = ({
  props,
  state
}) => (data, rgba) => {
  const {
...
const lightUp = ({
  ...
  const getArrayVal = arr => {
    let res = [];
    arr.forEach(item => {
      if (typeof item === "number") res = res.concat(getIndexVal(item));else if (typeof item === "string") res = res.concat(getPathVal(item));
      res = [...new Set(res)];
    });
    return res;
  };
...
const style = ({
  ...
  const styl = obj => {
    ...
    return Object.assign({}, styles, {
      name: `n${nodeName}/${iconIndex}`.toLowerCase()
    });
  };
...
export { buildOption, computeCanvasStyle, getContainerWidth, getMarkLine, getRenderIcon, getRenderLege, getRenderLink, getRenderNode, getTooltip, lightUp, resizeListener, setChartOption, setListeners, setRiverIconStyle, setRiverRgbaOpacity, style };