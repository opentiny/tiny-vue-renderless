...
const layout = (data, config, el) => {
  ...
  const {
    dataLinkRiverLinkMap,
    riverLinks
  } = initRiverLinks(links);
  calcRiverLinkPos({
    dataLinkRiverLinkMap,
    links,
    riverNodes,
  ...
  const riverIcons = calcRiverIcons({
    iconRadius,
    iconSpacing,
    riverNodes
  });
  ...
  riverIcons.forEach(riverIcon => {
    riverIcon.lp = [riverIcon.x - iconSpacing / 2 + iconPadding, riverIcon.y + labelOffset, iconSpacing - iconPadding * 2];
  });
  calcGradient({
    riverNodes,
    riverLinks,
  ...
  const leges = showLegend ? calcLegend({
    statusColor,
    statusName,
    cols,
    rows,
    ...
    if (node._children) {
      node._children.forEach(child => {
        const dataLink = links.find(({
          source,
          target
        }) => source === node.name && target === child.name);
...
const calcGradient = ({
  ...
  riverLinks.forEach(riverLink => {
    ...
    riverLink.g = [[0, 1], [last(sourceRiverNode.g[1]), first(targetRiverNode.g[1])]];
  });
};
const calcLegend = ({
  statusColor,
