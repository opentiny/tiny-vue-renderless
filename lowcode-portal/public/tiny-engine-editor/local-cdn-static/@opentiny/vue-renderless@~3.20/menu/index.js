...
const filterNodes = ({
  ...
  if (label !== "") {
    ...
    if (state.activedKeys) {
      ...
      nextTick(() => {
        api.setIsCurrent(state.data, {
          id: state.activedKeys
        });
      });
    }
  }
};
const getExpandedKeysPath = ({
  state
}) => keys => {
  const paths = [];
...
export { computeData, filterInput, filterNodes, getActivedKeysPath, getExpandedKeysPath, setAllNodeVisible, setIsCurrent, setNodeHidden };