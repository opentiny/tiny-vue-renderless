...
const initService = ({
  ...
  const noopFnCreator = propName => () => {
    if (propName) {
      return Promise.reject(new Error(`[TINY Error][Locale] Prop ${propName} is mandatory when the framework service is not used`));
    } else {
...
export { changeLocale, getLocales, initService, setText, switchLanguage };