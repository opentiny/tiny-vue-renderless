...
import { handleChange, setLink, tableMouseMove, tableChoose, toggleTablePanel, closeTablePanel, shouldShow, eventImg, eventClick, Active } from './index.js';
const api = ["state", "setLink", "handleChange", "tableMouseMove", "tableChoose", "toggleTablePanel", "closeTablePanel", "shouldShow", "eventImg", "eventClick", "Active"];
const renderless = (props, {
  ...
  let defaultToolBar = ["bold", "italic", "underline", "strike", "quote", "code", "codeBlock", "unorderedlist", "orderedlist", "taskList", "subscript", "superscript", "undo", "redo", "left", "center", "right", "formatClear", "link", "h-box",
  //
  "font-size",
  //
  "line-height",
  ...
  const defaultOptions = __spreadValues({
    extensions: [StarterKit == null ? void 0 : StarterKit.configure({
      // 开启多人协作功能要关闭默认的history模式
      history: true
    }), Table.configure({
      resizable: true
    }), TableCell, TableHeader, TableRow, Color, TextStyle, CustomImage, Highlight, Link, Underline, Subscript, Superscript, TaskList, TaskItem.configure({
      nested: true
    }), TextAlign.configure({
      types: ["heading", "paragraph"]
    }), CustomParagraph, CustomSize, CustomBackgroundColor, CodeBlockLowlight.extend({
      addNodeView() {
